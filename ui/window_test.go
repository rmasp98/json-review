...
func TestReturnsZeroDimensionIfNotResized(t *testing.T) {
	views := ui.NewWindow(panelRelativeWidth, border, tbBaseBuffer)
...
func assertEqualDimensions(t *testing.T, window ui.Window, expected [][]int) {
	for i := 0; i < 4; i++ {
		x0, y0, x1, y1 := window.GetDimensions(ui.ViewEnum(i))
		if x0 != expected[i][0] || y0 != expected[i][1] || x1 != expected[i][2] || y1 != expected[i][3] {
			t.Errorf(`Got "[%d,%d,%d,%d]" but expected "%+v" for %+v`, x0, y0, x1, y1, expected[i], ui.ViewEnum(i))
		}
	}
}
